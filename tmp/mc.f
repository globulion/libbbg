...
C
     ...
     &           DCONST=0.500D+00,MWALK=120000,NBIN=200)
      DIMENSION X(MWALK,3),E(MWALK),IMULT(MWALK),IBINPOP(NBIN,NBIN,NBIN)
C
C     --- INPUT PARAMETERS
C
      DATA XMIN/-10.0D+00/,XMAX/10.0D+00/
      DATA MAXWLK/MWALK/,MINWLK/250/,NORMWLK/20000/
      DATA NBLOCKS/100/,NSTEPS/1000/,ET/-0.497D+00/,NEQUIL/20/
      DATA ISEED/9183/
      DATA TAU/0.0005D+00/
C
...
C
     ...
     &              XMIN,XMAX,NBIN,IBINPOP,EBLOCK,EG)
      WRITE(901,1020) IBLOCK,EBLOCK,EG
 1020 FORMAT(I8,D22.8,D22.8)
 900  CONTINUE
C
      EAVE = EAVE / NBDONE
      EAV2 = EAV2 / NBDONE
...
C
         DO 110 K=1,M
            ACCEPT = ZERO
C
            CALL TRIALF(X(K,1),X(K,2),X(K,3),PSIX,ELX,FQX)
C
C           --- TRANSITION DUE TO DIFFUSION AND QUANTUM FORCE DRIFT ---
C
C            CALL GRAN(ISEED,VAR,GRX,GRY,GRZ)
            GRX = BOXMLR(ISEED,VAR)
            GRY = BOXMLR(ISEED,VAR)
            GRZ = BOXMLR(ISEED,VAR)
C
            Y(1) = X(K,1) + DCONST*TAU*FQX(1) + GRX
            Y(2) = X(K,2) + DCONST*TAU*FQX(2) + GRY
            Y(3) = X(K,3) + DCONST*TAU*FQX(3) + GRZ
C
            CALL TRIALF(Y(1),Y(2),Y(3),PSIY,ELY,FQY)
C
C           --- PERFORM GENERALIZED METROPOLIS SAMPLING ---
C
            Q = ZERO
C
            Q = Q + HALF*(FQX(1)+FQY(1))*(DC
     &                                *(FQX(1)-FQY(1))
     &                                -(Y(1)-X(K,1))) +
C
     &              HALF*(FQX(2)+FQY(2))*(DC
...
C
         IF(K.GT.MAXWLK) THEN
            NWALKERS = NORMWLK 
c           WRITE(*,*) '     ENSEMBLE OVERFLOW: RETURNING TO NORMWLK'
            GO TO 200
         ENDIF
         E(K) = E(IW)
         X(K,1) = X(IW,1)
...
c            WRITE(*,*) '     ENSEMBLE UNDERFLOW: RETURNING TO MINWLK'
            NWALKERS = MINWLK
         ENDIF
200      CONTINUE
C
...
C ==============================================================
      ...
      SUBROUTINE TRIALF(X,Y,Z,PSI,EL,FQ)
C
...
C ==============================================================
      ...
      SUBROUTINE SAVBIN(IBINPOP,NBIN)
C
...
C---------------------------------------------------------------
      ...
      DO 120 J=1,NBIN
      DO 120 K=1,NBIN
         WRITE(111,1050) I,J,K,IBINPOP(I,J,K)
 120  CONTINUE
 1050 FORMAT(3(I6),I10)
      RETURN
      END
C ==============================================================
