...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
 ...
 Jae-Ho Chung corrected a few lines:
     1. fcn(x, fjac=fjac, **functkw) ==> eval(fcn+"(x, fjac=fjac, **functkw)") : two places
     2. x[ipvt] = wa ==> numpy.put(x, ipvt, wa) : Following the warning of the Spyder

 Bartosz Błasiak has eliminated a bug (8 Nov 2013)
     with <err> argument: if err is None -> err should be 1.0D+00. 
     The following line in function models is added:
...
#			[status, f] = fcn(x, fjac=fjac, **functkw)
			[status, f] = eval(fcn+"(x, fjac=fjac, **functkw)") # Corrected by Jae-Ho Chung
			if self.damp > 0:
...
#			return fcn(x, fjac=fjac, **functkw)
			return(eval(fcn+"(x, fjac=fjac, **functkw)")) # Corrected by Jae-Ho Chung
...
#		x[ipvt] = wa
		numpy.put(x, ipvt, wa) # Corrected by Jae-Ho Chung
		return (r, x, sdiag)
...
def mpfit_gaussian(p, fjac=None, x=None, y=None, err=None):
...
def gaussian(x, p):
    # p = [[area,center,width],...[back],[slope]]
    nparam = len(p)
    model = 0.
    for i in range(nparam//3):
        tmp = p[3*i]/(p[3*i+2]*numpy.sqrt(2*numpy.pi))
        model += tmp/numpy.exp(0.5*(x-p[3*i+1])**2/p[3*i+2]**2)
    if nparam%3 == 1: model += p[-1] # flat background
    elif nparam%3 == 2: model += p[-2]+p[-1]*x # linear background
    return(model)
    
def mpfit_polynomial(p, fjac=None, x=None, y=None, err=None):
    numpy.seterr(over='ignore')
    model = polynomial(x,p)
    status = 0
    if err is None: err=1.
    return([status, (y-model)/err])
...
def dpolynomial(x, p):
    ...
    model = 0.
    for i in range(1,nparam):
        model = model + i*p[i]*x**(i-1)
    return(model)

def mpfit_sinusoidal(p, fjac=None, x=None, y=None, err=None):
    numpy.seterr(over='ignore')
    model = sinusoidal(x,p)
    status = 0
    if err is None: err=1.
    return([status, (y-model)/err])

def sinusoidal(x, p):
    # p = [[amp,node,freq],...[back],[slope]]
    nparam = len(p)
    model = 0.
    for i in range(nparam//3):
        model += p[3*i]*numpy.sin((x-p[3*i+1])/p[3*i+2])
    if nparam%3 == 1: model += p[-1] # flat background
    elif nparam%3 == 2: model += p[-2]+p[-1]*x # linear background
    return(model)

def mpfit_new(p, fjac=None, x=None, y=None, err=None):
    numpy.seterr(over='ignore')
    model = new(x,p)
    status = 0
    if err is None: err=1.
...
def new(x,p):
    # p = [[A,xo,C,B],...[back],[slope]]
    nparam = len(p)
    model = 0.
    for i in range(nparam//4):
        model += p[4*i]*(1.-p[4*i+3]*numpy.exp(-p[4*i+2]*(x-p[4*i+1])))/\
                        (1.+p[4*i+3]*numpy.exp(-p[4*i+2]*(x-p[4*i+1])))
    if nparam%3 == 1: model += p[-1] # flat background
    elif nparam%3 == 2: model += p[-2]+p[-1]*x # linear background
    return(model)
